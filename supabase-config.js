...
window.NovusDB.leanInitiatives = {
  ...
  add: (item) =>
    _sb.from('lean_initiatives').insert({
      ...
      est_hours_saved:      item.estHoursSaved        || 0,
      actual_hours_saved:   item.actualHoursSaved      || 0,
      est_material_savings: item.estMaterialSavings    || 0,
      notes:                item.notes                || '',
      week_flagged:         item.weekFlagged           || '',
    }),

  updateStatus: (id, status) =>
    _sb.from('lean_initiatives').update({ status }).eq('id', id),
